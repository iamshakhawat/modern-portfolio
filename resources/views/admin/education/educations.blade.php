        ...
        <div class="max-w-7xl mx-auto">
            <div
                ...
                <div class="px-8 py-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                    <h2 class="text-2xl font-extrabold text-gray-900 dark:text-white">Education List</h2>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                            @forelse($educations as $index => $education)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100 font-semibold">
                                        @if($education->icon)
                                            <i class="{{ $education->icon }} text-2xl"></i>
                                        @else
                                    ...
                                    <td class="px-4 py-3">
                                        @if ($education->status === 'Passed')
                                            <span class="px-3 py-1 rounded-full text-xs font-bold bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400">Passed</span>
                                        @else
                                            <span class="px-3 py-1 rounded-full text-xs font-bold bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400">{{ $education->status }}</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 text-xs text-gray-700 dark:text-gray-300">
                                        ...
                                        <button type="button"
                                            onclick="deleteData('{{ route('admin.educations.delete', $education->id) }}')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-8 text-center text-gray-400 dark:text-gray-500">No educations found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
