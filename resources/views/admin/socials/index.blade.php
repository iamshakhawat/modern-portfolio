        ...
        <div class="max-w-7xl mx-auto">
            <div
                ...
                <div class="px-8 py-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                    <h2 class="text-2xl font-extrabold text-gray-900 dark:text-white">Socials List</h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Color</th>
                                <th class="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">URL</th>
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                            @forelse($socials as $index => $social)
                                <tr>
                                    <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">
                                        {{ $socials->firstItem() + $index }}</td>
                                    <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 capitalize">
                                        {{ $social->name }}</td>
                                    <td class="px-4 py-3">
                                        <span class="inline-flex items-center">
                                            <i class="fab {{ $social->icon }} text-xl" style="color: {{ $social->color }}"></i>
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 flex items-center">
                                        @if ($social->color)
                                            <span
                                                ...
                                                style="background: {{ $social->color }}"></span>
                                            <span class="ml-2 text-xs font-mono text-gray-500">{{ $social->color }}</span>
                                        @else
                                            <span class="text-gray-400 text-sm text-center">N/A</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 text-xs text-gray-900 dark:text-gray-100">
                                        <a href="{{ $social->url }}" target="_blank" class="underline text-blue-600 dark:text-blue-400">{{ $social->url }}</a>
                                    </td>
                                    <td class="px-4 py-3">
                                        @if ($social->status)
                                            <span
                                    ...
                                    <td class="px-4 py-3 flex space-x-2">
                                        ...
                                        <button type="button"
                                            onclick="deleteData('{{ route('admin.socials.delete', $social->id) }}')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-8 text-center text-gray-400 dark:text-gray-500">No socials found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
