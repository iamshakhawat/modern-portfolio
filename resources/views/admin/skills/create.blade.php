...
@push('css')
    <style>
        ...
        /* Dark Mode overrides for the slider track if needed */
        .dark .slider {
        ...
        <div class="max-w-full mx-auto">
            <div
                ...
                <div class="px-8 py-6 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                    <div>
                        <h2 class="text-2xl font-extrabold text-gray-900 dark:text-white">Add New Skill</h2>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Define a technical skill for your portfolio.
                        </p>
                    </div>
                    ...
                    <form action="{{ route('admin.skills.store') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="name"
                                        class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Skill
                                        Name</label>
                                ...
                                <div>
                                    ...
                                    <select name="category" id="category" required
                                        ...
                                        <option value="frontend" {{ old('category') == 'frontend' ? 'selected' : '' }}>
                                            Frontend Development</option>
                                        <option value="backend" {{ old('category') == 'backend' ? 'selected' : '' }}>Backend
                                            Development</option>
                                        <option value="programming"
                                            {{ old('category') == 'programming' ? 'selected' : '' }}>Programming Languages
                                        </option>
                                        <option value="tools" {{ old('category') == 'tools' ? 'selected' : '' }}>DevOps &
                                            Tools</option>
                                    </select>
                                    @error('category')
                                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            ...
                            <!-- Asset Type Toggle -->
                            <div
                                class="bg-gray-50 dark:bg-gray-700/50 p-5 rounded-2xl border border-gray-200 dark:border-gray-600">


                                <!-- Icon Input Group (Visible by default) -->
                                <div id="iconSection"
                                    class="{{ old('type') ? 'hidden-section' : 'grid' }} grid-cols-1 md:grid-cols-2 gap-6 transition-all duration-300">
                                    <div>
                                        <label
                                            class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">FontAwesome
                                            Class</label>
                                        <div class="relative group">
                                            <div
                                                class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                                <i id="iconPreview"
                                                    class="{{ old('icon', 'fa-solid fa-circle-question') }} text-indigo-500 text-lg"
                                                    style="color: {{ old('color', '#4F46E5') }}"></i>
                                            </div>
                                        ...
                                        @error('icon')
                                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                                        @enderror
                                        <p class="text-[10px] text-gray-500 mt-2 italic px-1">Example: fa-brands fa-php,
                                            fa-solid fa-database</p>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">Brand
                                            Color</label>
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <span id="colorHex"
                                                class="text-sm font-mono text-gray-500">{{ strtoupper(old('color', '#4F46E5')) }}</span>
                                        </div>
                                        @error('color')
                                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                                        @enderror
                            ...
                            <div class="flex items-center justify-between py-4 px-2">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Determine if this skill is visible
                                        on your public profile.</p>
                                </div>
                                <div class="flex items-center space-x-4">
                                    <span id="statusText"
                                        class="text-xs font-black uppercase tracking-tighter {{ old('status', 'on') === 'on' ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400' }}">
                                        {{ old('status', 'on') === 'on' ? 'Active' : 'Inactive' }}
                                    </span>
                                    <label class="switch">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                iconInput.addEventListener('input', (e) => {
                    ...
                    // Ensure the text color stays from our indigo class unless specified otherwise
