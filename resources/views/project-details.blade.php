                    ...
                    <header class="space-y-4">
                        ...
                        <div class="flex items-center space-x-4 text-sm text-slate-500 dark:text-slate-400">
                            <div class="flex items-center">
                                @if ($project->user && $project->user->photo)
                                    <img src="{{ $project->user->photo }}" alt="Author Photo"
                                ...
                                <span>Project by <span
                                        class="font-medium text-slate-900 dark:text-slate-200">{{ $project->user->name }}</span></span>
                            </div>
                            <span class="hidden sm:inline text-slate-300">|</span>
                            <time class="flex items-center"><i class="fa-regular fa-calendar-check mr-2"></i>
                                {{ $project->created_at->format('F j, Y') }}</time>
                    ...
                    <div
                        ...
                        <dl class="space-y-4">
                            <div
                                ...
                                <dd class="font-semibold text-slate-900 dark:text-slate-100">{{ $project->client }}
                                </dd>
                            </div>
                            <div class="flex justify-between items-center py-2 text-sm">
                                <dt class="text-slate-500 dark:text-slate-400">Duration</dt>
                                <dd class="font-semibold text-slate-900 dark:text-slate-100">{{ $project->duration }}
                                </dd>
                            </div>
                            <div class="flex justify-between items-center py-2 text-sm">
                                <dt class="text-slate-500 dark:text-slate-400">Rating</dt>
                                <dd class="font-semibold text-slate-900 dark:text-slate-100">
                                    <i class="fa-solid fa-star text-yellow-400 mr-1"></i> {{ $project->rating }}/5
                                </dd>
                            </div>
                            <div class="flex justify-between items-center py-2 text-sm">
                                ...
                                <dd class="font-semibold w-2/3 text-slate-900 text-right dark:text-slate-100">
                                    @foreach ($project->categories as $category)
                                        <span
                                            class="px-3 inline-block mb-3 py-1 bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 rounded-full text-xs font-bold capitalize tracking-wider">{{ $category->name }}</span>
                                    @endforeach
                                </dd>
                            </div>
                        </dl>
                        ...
                        <div class="grid grid-cols-3 gap-6">
                            ...
                            @foreach ($project->skills as $skill)
                                <div class="flex flex-col items-center group">
                                    <div
                                        class="w-12 h-12 rounded-2xl bg-red-50 dark:bg-red-900/10 flex items-center justify-center text-[{{ $skill->color }}] text-2xl group-hover:scale-110 transition-transform">
                                        <i class="{{ $skill->icon }}"></i>
                                    </div>
                                    <span
                                        class="text-[10px] font-bold mt-2 uppercase tracking-tighter text-slate-400">{{ $skill->name }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
